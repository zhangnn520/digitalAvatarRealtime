...
pool_executor: ProcessPoolExecutor = None


def get_pool_executor():
    global pool_executor
    if pool_executor is None:
        pool_executor = ProcessPoolExecutor(max_workers=Settings().max_workers)
...
async def inf_video(vid: str, video_name: str, video_bytes: bytes, audio_bytes: bytes):
    ...
    frames_ndarray, ds_feature = await asyncio.gather(
        asyncio.get_running_loop().run_in_executor(pool_executor,
    ...
    res_video_frames_data_pad, res_video_landmark_data_pad = await asyncio.get_running_loop().run_in_executor(
        pool_executor, _get_frames_landmarks_pad, frames_ndarray, video_landmark_data, res_frame_length)
    ...
    ref_video_frame, video_size = await asyncio.get_running_loop().run_in_executor(
        pool_executor, _pick5frames, res_video_frames_data_pad, res_video_landmark_data_pad, resize_w, resize_h)
